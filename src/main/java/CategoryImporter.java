...
public class CategoryImporter {
    public static void etl(final KalturaToMpxImporter importer) throws KalturaApiException {
        ...
        List<Category> transformedCategories = oldCategories.stream().map(KalturaMpxConverter.convertCategory).collect(Collectors.toList());
        ...
        List<Category> newCategoriesWithParent = newCategories.stream().map(c -> {
            KalturaCategory parentInOldCategories = oldCategories.stream()
                    .filter(oldCat -> KalturaMpxConverter.importId(oldCat.id).equals(c.getGuid()))
                    .findAny()
                    .get();
            if (parentInOldCategories.parentId > 0) {
                // if no root node
                Category parentInNewCategories = newCategories.stream()
                        .filter(newCat -> newCat.getGuid().equals(KalturaMpxConverter.importId(parentInOldCategories.parentId)))
                        .findAny()
                        .get();
